...
export function AlertsCenterView() {
  ...
  const activeTabOverride = useMemo(() => {
    if (activeTab === "bloquante") return { niveau: "bloquante" }
    if (activeTab === "analyser") return { niveau: "analyser" }
    if (activeTab === "nouvelle") return { statut: "nouvelle" }
    return {}
  }, [activeTab])
  ...
  const {
    ...
    ({ skip, limit }) =>
      alertService.getAlertsPage(filters, { skip, limit }, { q: debouncedSearchTerm, ...activeTabOverride }),
    [filters, debouncedSearchTerm, activeTab],
    { pageSize: 20 }
  )
  const filteredAlerts = alerts
  const loadAlerts = refetchAlerts

  // Compteurs des onglets rapides : portée = filtres actifs + recherche, pas l'onglet lui-même
  const [tabCounts, setTabCounts] = useState({ total: 0, bloquantes: 0, analyser: 0, nouvelles: 0 })
  useEffect(() => {
    let cancelled = false
    Promise.all([
      alertService.getAlertsPage(filters, { skip: 0, limit: 1 }, { q: debouncedSearchTerm }),
      alertService.getAlertsPage(filters, { skip: 0, limit: 1 }, { q: debouncedSearchTerm, niveau: "bloquante" }),
      alertService.getAlertsPage(filters, { skip: 0, limit: 1 }, { q: debouncedSearchTerm, niveau: "analyser" }),
      alertService.getAlertsPage(filters, { skip: 0, limit: 1 }, { q: debouncedSearchTerm, statut: "nouvelle" }),
    ]).then(([total, bloquantes, analyser, nouvelles]) => {
      if (cancelled) return
      setTabCounts({ total: total.total, bloquantes: bloquantes.total, analyser: analyser.total, nouvelles: nouvelles.total })
    })
    return () => {
      cancelled = true
    }
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        {loading ? (
          ...
          /* Affichage en Tableau Épuré 5 Colonnes avec Progressive Disclosure */
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead className="border-b border-slate-200/80 bg-slate-50/70 text-xs font-semibold text-slate-500 uppercase tracking-wider dark:border-slate-800 dark:bg-slate-800/50 dark:text-slate-400">
                <tr>
                  <th className="py-3.5 pl-4 pr-3">Niveau & Réf</th>
                  <th className="px-3 py-3.5">Client</th>
                  <th className="px-3 py-3.5">Type & Motif</th>
                  <th className="px-3 py-3.5 text-center">Score</th>
                  <th className="py-3.5 pl-3 pr-4 text-right">Statut & Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800/80 text-slate-700 dark:text-slate-200">
                {filteredAlerts.map((a) => {
                  ...
                  return (
                    <tr key={a.id || a.ref} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors group">
                      {/* Col 1: Niveau & Réf */}
                      <td className="py-3.5 pl-4 pr-3">
                        <div className="flex items-center gap-2">
                          <Badge variant="outline" className={cn("border flex items-center gap-1 text-2xs font-semibold", levelConf.badgeClass)}>
                            <LevelIcon className="h-3 w-3" />
                            {levelConf.label}
                          </Badge>
                          <span className="font-mono text-xs font-semibold text-slate-900 dark:text-white">{a.ref}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-3 py-3.5">
                        <div>
                          <button
                            onClick={() => navigateTo("Client 360°", { clientId: a.clientId })}
                            className="font-semibold text-slate-900 dark:text-white hover:text-indigo-600 dark:hover:text-indigo-400 transition flex items-center gap-1 text-left"
                          >
                            <span>{a.client}</span>
                            <ExternalLink className="h-3 w-3 text-slate-400" />
                          </button>
                          {a.clientId && (
                            <span className="text-2xs font-mono text-slate-400">ID: {a.clientId.slice(0, 8)}...</span>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="px-3 py-3.5">
                        <div className="max-w-md">
                          <p className="font-medium text-slate-800 dark:text-slate-200 leading-tight">
                            {a.type} <span className="text-2xs text-indigo-600 dark:text-indigo-400 font-medium">({a.module})</span>
                          </p>
                          {a.facteurs && a.facteurs.length > 0 && (
                            <p className="text-2xs text-slate-500 dark:text-slate-400 truncate mt-0.5" title={a.facteurs[0]}>
                              {a.facteurs[0]}
                            </p>
                          )}
                        </div>
                      ...
                      <td className="px-3 py-3.5 text-center">
                        <span
                          className={cn(
                            "inline-block rounded-full px-2.5 py-0.5 text-xs font-bold font-mono",
                            a.score >= 75
                      ...
                      <td className="py-3.5 pl-3 pr-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <RowActionDropdown
                            ...
                            actions={[
                              {
                                label: "Détail de l'alerte",
                                icon: <ExternalLink className="h-3.5 w-3.5" />,
                                onClick: () => setSelectedAlertForModal(a),
                              },
                              {
                                label: "Consulter Client 360°",
                              ...
                              },
                              {
                                ...
                                onClick: () => handleStatusChange(a, "en_cours"),
                                disabled: a.status === "en_cours" || updatingId === (a.id || a.ref),
                              },
                              {
                                label: "Marquer comme Clôturée",
                                icon: <CheckCircle2 className="h-3.5 w-3.5" />,
                                onClick: () => handleStatusChange(a, "cloturee"),
                                disabled: a.status === "cloturee" || updatingId === (a.id || a.ref),
                              },
                              {
                                label: "Classer sans suite",
                                icon: <Archive className="h-3.5 w-3.5" />,
                                onClick: () => handleStatusChange(a, "classee"),
                                disabled: a.status === "classee" || updatingId === (a.id || a.ref),
                                danger: true,
                              },
                            ]}
                          />
        ...
        {/* Pied de tableau : pagination réelle côté serveur */}
